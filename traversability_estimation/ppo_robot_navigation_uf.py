...
def reset_single_frame(stacked_frames, state, stack_size, number):
    ...
    # if (state.shape[1] > 10):
    #     fig, ax = plt.subplots()
    #     im = ax.imshow(state[number])
    #     ax.axis('off')
    #     plt.show()
    #
    #     for i in range(0, stack_size):
    #         print("stack: " + str(i + 1))
    #         for j in range(0, num_envs):
    #             print("worker: " + str(j + 1))
    #             fig, ax = plt.subplots()
    #             im = ax.imshow(stacked_frames[i][j])
    #             ax.axis('off')
    #             plt.show()

    for i in range(0, stack_size, 1):
        #stacked_frames.append(state)
        stacked_frames[i][number] = state[number]

    # if (state.shape[0] > 10):
    #     fig, ax = plt.subplots()
    #     im = ax.imshow(map_state[i])
    #     ax.axis('off')
    #     plt.show()
    #
    #     for i in range(0, stack_size):
...
def test_env(vis=False):
    ...
    map_state, depth_state,goal_state  = envs.reset_test()
    ...
    with torch.no_grad():
        while not done:
            ...
            next_map_state, next_depth_state, next_goal_state, reward, done, _ = envs.step_test(action)
...
def ppo_update(ppo_epochs, mini_batch_size, map_states, depth_states, goal_states, hidden_states_h, hidden_states_c, actions, log_probs, returns, advantages, clip_param=0.2):
    for _ in range(ppo_epochs):
        for  map_state, depth_state, goal_state, hidden_state_h, hidden_state_c, action, old_log_probs, return_, advantage in ppo_iter(mini_batch_size,  map_states, depth_states, goal_states, hidden_states_h, hidden_states_c, actions, log_probs,
            ...
            dist, value, hidden_state_c, hidden_state_h = model( map_state.to(device), depth_state.to(device), goal_state.to(device),  hidden_state_h, hidden_state_c)
...
while frame_idx < max_frames and not early_stop:
    ...
    with torch.no_grad():
        for _ in range(num_steps):
            ...
            map_state = torch.FloatTensor(map_state).to('cpu')
            depth_state = torch.FloatTensor(depth_state).to('cpu')
            goal_state = torch.FloatTensor(goal_state).to('cpu')

            hidden_states_h.append(hidden_state_h)
            hidden_states_c.append(hidden_state_c)

            dist, value, hidden_state_h, hidden_state_c = model(map_state.to(device), depth_state.to(device), goal_state.to(device), hidden_state_h, hidden_state_c)
            ...
            for i in range(0, num_envs):
                if(done[i]==False):
                    _ , stacked_map_frames = reset_single_frame(stacked_map_frames, next_map_state , stack_size, i)
                    _ , stacked_depth_frames = reset_single_frame(stacked_depth_frames, next_depth_state, stack_size, i)
                    _ , stacked_goal_frames = reset_single_frame(stacked_goal_frames, next_goal_state,stack_size , i)
            ...
            if frame_idx % 5000 == 0:
                ...
                for _ in range(5):
                    ...
                    # print("test_reward: " + str(test_reward))
                    # print("test_lenght: " + str(test_lenght))
                    # print("test_log_probs: " + str(test_log_probs))
                    # print("test_values: " + str(test_values))
                    # print("test_entropy: " + str(test_entropy))
                ...
                # print("mean_test_rewards: " + str(mean_test_rewards))
                # print("mean_test_lenghts: " + str(mean_test_lenghts))
                # print("mean_test_log_probs: " + str(mean_test_log_probs))
                # print("mean_test_values: " + str(mean_test_values))
                # print("mean_test_entropy: " + str(mean_test_entropy))
            ...
            next_map_state = torch.FloatTensor(next_map_state).to('cpu')
            next_depth_state = torch.FloatTensor(next_depth_state).to('cpu')
            next_goal_state = torch.FloatTensor(next_goal_state).to('cpu')

    model.train()

    _, next_value, hidden_state_h, hidden_state_c = model(next_map_state.to(device), next_depth_state.to(device), next_goal_state.to(device), hidden_state_h, hidden_state_c)
    ...
    ppo_update(ppo_epochs, mini_batch_size, map_states, depth_states, goal_states, hidden_states_h, hidden_states_c , actions, log_probs, returns, advantage)
