...
def worker(remote, parent_remote, env_fn_wrapper):
    ...
    while True:
        ...
        if cmd == 'step':
            ...
            obs_1, obs_2 = env.get_state()
            remote.send((obs_1, obs_2))
        elif cmd == 'reset_task':
            obs_1, obs_2 = env.reset_task()
            remote.send((obs_1, obs_2))
        elif cmd == 'close':
            remote.close()
            break
...
class VecEnv(object):
    ...
    def step_wait(self):
        ...
        Returns (obs_1, obs_2, rews, dones, infos):
         - obs_1, obs_2: an array of observations, or a tuple of
...
class SubprocVecEnv(VecEnv):
    ...
    def reset(self):
        ...
        results = [remote.recv() for remote in self.remotes]
        obs_1, obs_2 = zip(*results)
        return np.stack(obs_1), np.stack(obs_2)
